...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($errors)) {
        ...
        try {
            $stmt = $conn->prepare(
                "INSERT INTO children (user_id, name, date_of_birth, gender, blood_type, created_at, updated_at)
                 VALUES (?, ?, ?, ?, ?, NOW(), NOW())"
            );
            $stmt->bind_param("issss", $user_id, $name, $date_of_birth, $gender, $blood_type);

            if ($stmt->execute()) {
                $success = 'Data anak berhasil ditambahkan!';
                log_activity($user_id, 'ADD_CHILD', 'Tambah anak: ' . $name);
                // Clear form
                $_POST = [];
            } else {
                $error = 'Gagal menambahkan data anak: ' . $stmt->error;
            }
            $stmt->close();
        } catch (Exception $e) {
...
<head>
    ...
    <title>Tambah Data Anak - Reminder Imunisasi</title>
...
<body>
    <div class="form-container">
        <div class="form-header">
            ...
            <h2><i class="fas fa-plus-circle me-2"></i>Tambah Data Anak</h2>
            <p>Tambahkan informasi lengkap tentang anak Anda</p>
        </div>
        ...
        <?php if (!empty($success)): ?>
            ...
            <div style="text-align: center; margin-top: 20px;">
                <a href="data_anak.php" class="btn-submit" style="display: inline-block; width: auto; margin-right: 10px;">
                    <i class="fas fa-arrow-left me-2"></i>Kembali
                </a>
                <button onclick="location.reload();" class="btn-submit" style="width: auto;">
